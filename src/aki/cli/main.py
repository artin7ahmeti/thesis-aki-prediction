...
from aki.labels.kdigo import build_kdigo_concepts_and_labels
...
app = typer.Typer(
    ...
    help="AKI prediction pipeline — stages are run in this order: "
...
def cmd_stage() -> None:
    ...
    write_manifest(cfg.raw_dir, paths.artifacts / "data_manifest.json")

    params = {"raw_dir": str(cfg.raw_dir).replace("\\", "/")}
    with DuckDBSession(
        memory_limit=db_cfg.get("memory_limit", "16GB"),
        threads=db_cfg.get("threads", 8),
    ) as conn:
...
@app.command("cohort")
def cmd_cohort() -> None:
    """Build cohort.cohort + cohort.landmarks."""
    cfg = load_configs()
...
@app.command("labels")
def cmd_labels() -> None:
    """Run KDIGO concepts + label SQL and export parquet."""
    cfg = load_configs()
    with DuckDBSession() as conn:
        build_kdigo_concepts_and_labels(conn, cfg)
...
def cmd_qa() -> None:
    ...
    _ = cfg  # currently unused beyond session defaults
...
def cmd_pipeline(
    ...
    """End-to-end: stage -> cohort -> labels -> features -> train -> minimal -> evaluate -> explain -> report."""
    cfg = load_configs()
    if not skip_stage:
        cmd_stage()
    cmd_cohort()
