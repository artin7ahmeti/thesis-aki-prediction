...
def tune_model(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        logger.info(
            f"HPO {model_name}: trial {trial.number + 1}/{n_trials} started "
            f"({len(folds)} patient-grouped folds)"
        )
        for step, (tr_idx, va_idx) in enumerate(folds):
            ...
            if y_va.nunique() < 2:
                logger.info(
                    f"HPO {model_name}: trial {trial.number + 1}/{n_trials} "
                    f"fold {step + 1}/{len(folds)} skipped (single class)"
                )
                continue
            try:
            ...
            logger.info(
                f"HPO {model_name}: trial {trial.number + 1}/{n_trials} "
                f"fold {step + 1}/{len(folds)} "
                f"AUROC={auroc_folds[-1]:.4f} AUPRC={auprc_folds[-1]:.4f} "
                f"running_AUROC={float(np.mean(auroc_folds)):.4f}"
            )
            ...
            if trial.should_prune():
                logger.info(
                    f"HPO {model_name}: trial {trial.number + 1}/{n_trials} "
                    f"pruned after fold {step + 1}/{len(folds)}"
                )
